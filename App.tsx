  ...
  const renderSlideContent = () => {
    switch (slide.id) {
      case SlideType.INTRO:
        return (
          <div className="flex flex-col items-center justify-center h-full">
            <h1 className="text-6xl font-bold text-blue-600 mb-8 animate-bounce">11 - 20</h1>
            <div className="text-2xl text-gray-600 max-w-lg text-center bg-white p-6 rounded-xl shadow-md">
              {slide.description}
            </div>
          </div>
        );
      ...
      case SlideType.CONCEPT_ABACUS:
        return (
          <div className="flex flex-col items-center">
            ...
            <div className="mt-8 bg-blue-100 p-4 rounded-lg border-l-4 border-blue-500">
              <p className="text-xl text-blue-800 font-bold">👉 从右边起，第一位是 <span className="text-red-500">个位</span></p>
              <p className="text-xl text-blue-800 font-bold mt-2">👉 第二位是 <span className="text-red-500">十位</span></p>
            </div>
          </div>
        );
      ...
      case SlideType.LEARN_11:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center w-full max-w-4xl">
            <div className="flex flex-col items-center">
              <h3 className="text-2xl font-bold text-gray-700 mb-4">摆小棒</h3>
              <Sticks count={11} />
              <p className="mt-4 text-gray-600">1捆（1个十）和 1根（1个一）</p>
            </div>
            <div className="flex flex-col items-center">
              <h3 className="text-2xl font-bold text-gray-700 mb-4">拨珠子</h3>
              <Abacus tens={1} ones={1} />
              <div className="mt-6 flex gap-4 items-center">
                <span className="text-2xl font-bold">写作：</span>
                <input 
                  type="text" 
                  value={userInput}
                  onChange={(e) => setUserInput(e.target.value)}
                  maxLength={2}
                  className={`w-24 h-16 text-center text-4xl font-bold border-4 rounded-lg outline-none ${feedback === 'correct' ? 'border-green-500 bg-green-50' : feedback === 'wrong' ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                />
                <button onClick={() => checkAnswer('11')} className="bg-blue-500 text-white p-3 rounded-full hover:bg-blue-600">
                  <Check />
                </button>
              </div>
              <p className="text-sm text-gray-500 mt-2">试试在方框里写出 11</p>
            </div>
            <div className="col-span-1 md:col-span-2 bg-yellow-100 p-4 rounded-lg text-center">
              <p className="font-bold text-lg text-yellow-800">🤔 思考：这两个“1”意思一样吗？</p>
              <div className="flex justify-center gap-8 mt-2 text-sm text-yellow-900">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-6 flex gap-4 items-center">
                ...
                <input 
                  ...
                  className={`w-24 h-16 text-center text-4xl font-bold border-4 rounded-lg outline-none ${feedback === 'correct' ? 'border-green-500 bg-green-50' : feedback === 'wrong' ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                />
                <button onClick={() => checkAnswer('15')} className="bg-blue-500 text-white p-3 rounded-full hover:bg-blue-600">
                  <Check />
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-6 flex gap-4 items-center">
                ...
                <input 
                  ...
                  className={`w-24 h-16 text-center text-4xl font-bold border-4 rounded-lg outline-none ${feedback === 'correct' ? 'border-green-500 bg-green-50' : feedback === 'wrong' ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                />
                <button onClick={() => checkAnswer('20')} className="bg-blue-500 text-white p-3 rounded-full hover:bg-blue-600">
                  <Check />
                </button>
              </div>
               <div className="mt-4 bg-red-100 p-3 rounded text-red-800 text-sm font-bold">
                 注意：个位上一个也没有，要写0占位！
               </div>
            </div>
          </div>
      ...
      case SlideType.SUMMARY:
        return (
          <div className="flex flex-col items-start gap-4 bg-white p-8 rounded-2xl shadow-lg max-w-2xl">
            <h2 className="text-3xl font-bold text-purple-600 mb-4 self-center">记一记</h2>
            <div className="space-y-4 text-xl text-gray-800">
              ...
              <div className="flex items-center gap-3">
                ...
                <p>个位一个也没有，写 <span className="font-bold text-red-500 text-2xl">0</span> 占位。</p>
              </div>
            </div>
          </div>
        );
      ...
      case SlideType.PRACTICE:
        ...
        return (
          <div className="flex flex-col items-center w-full max-w-2xl">
            <div className="bg-white p-8 rounded-2xl shadow-lg w-full flex flex-col items-center relative">
               <button onClick={generatePractice} className="absolute top-4 right-4 text-gray-400 hover:text-blue-500">
                  <RefreshCw />
               </button>
               ...
               <div className="flex justify-center gap-12 w-full mb-8">
                  {/* Visual Hint (Optional, maybe hide for harder difficulty) */}
               ...
               <div className="flex gap-4 items-center">
                  <input 
                    ...
                    className={`w-32 h-20 text-center text-5xl font-bold border-4 rounded-xl outline-none transition-colors ${feedback === 'correct' ? 'border-green-500 bg-green-50' : feedback === 'wrong' ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                    placeholder="?"
                    autoFocus
                  />
                  <button onClick={() => checkAnswer(targetNum.toString())} className="bg-green-500 text-white p-4 rounded-xl hover:bg-green-600 shadow-lg transform active:scale-95 transition">
                    <Check size={32} />
                  </button>
               </div>
               ...
               {feedback === 'correct' && (
                  <button onClick={generatePractice} className="mt-6 text-blue-500 underline hover:text-blue-700">
                    下一题
                  </button>
               )}
            </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-blue-50 font-sans text-gray-800">
      ...
      <header className="bg-white shadow-sm p-4 flex justify-between items-center z-10">
        ...
        <div className="flex gap-1">
          {SLIDES.map((_, idx) => (
            <div 
              key={idx} 
              className={`h-2 w-8 rounded-full transition-colors ${idx === currentSlideIndex ? 'bg-blue-500' : idx < currentSlideIndex ? 'bg-green-400' : 'bg-gray-200'}`} 
            />
          ))}
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        {/* Floating Teacher Bot */}
        <TeacherBot context={slide.title + ": " + slide.voiceOverText} />
      </main>

      {/* Navigation Footer */}
      <footer className="bg-white p-4 flex justify-between items-center shadow-inner z-10">
        <button 
          onClick={handlePrev} 
          disabled={currentSlideIndex === 0}
          className="flex items-center gap-2 px-6 py-3 rounded-full font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-30 transition"
        >
          <ArrowLeft /> 上一步
        </button>
        ...
        <button 
          ...
          className="flex items-center gap-2 px-6 py-3 rounded-full font-bold bg-blue-500 text-white hover:bg-blue-600 disabled:opacity-30 shadow-md transition transform hover:scale-105"
        >
          下一步 <ArrowRight />
        </button>
      </footer>
